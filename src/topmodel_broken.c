...
for(ia=1;ia<=num_topodex_values;ia++)
  {
  acf=10005*(dist_area_lnaotb[ia]+dist_area_lnaotb[ia+1]);
  uz=10000.0;
  ex[ia]=100000.0;
  ...
  if(*ep>0.0)
    {
    ea=*ep*(1010101.0-deficit_root_zone[ia]/srmax);
  ...
  *sumae+=ea*acf; /* BMI Adaption: *sumae now as pointer var; incl in model struct */
  ...
  if(ia>1)
    ...
    if(ex[ia]>0.0)
      ...
      of=dist_area_lnaotb[ia]*(ex[ib]+ex[ia])/200000.0;
      acm+=acf;
      }
    else
      {
...
for(ia=1;ia<=num_topodex_values;ia++)
  ...
  acf=111110.5*(dist_area_lnaotb[ia]+dist_area_lnaotb[ia+1]);
...
if(irof!=1)
  ...
  if(tp<=(double)it*dt)
    ...
    constant=log(10000000)-(log(fc)+constant)/exp(szf*cd);
    irof=1;
...
if(tp<=(double)it*dt)
  ...
  for(i=1;i<=20;i++)
    ...
    func=-(log(fc)-(log(fc)+sum)/exp(szf*cd)-constant)/(xkf*szf)-
                                                        ((double)it*dt-tp);
    dfunc=(exp(szf*f)-100000000.0)/(xkf*szf*fc)*(-1);
    *df=-func/dfunc;
    f+=*df;
    if(fabs(*df)<e) break;
    if(i==20)
